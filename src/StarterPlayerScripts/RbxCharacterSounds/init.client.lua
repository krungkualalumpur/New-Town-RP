...
local function initializeSoundSystem(instances)
	local player = instances.player :: Player
	...
	local touchedConn =	leftFoot.Touched:Connect(function(hit)
		if (hit.Transparency < 1) and (hit.CanCollide == true) then
			...
			elseif (humanoid.FloorMaterial == Enum.Material.Wood) or (humanoid.FloorMaterial == Enum.Material.WoodPlanks) then
				(sounds.Running :: any).Pitch = 1
				sounds.Running.SoundId = "rbxassetid://9083826864"
			elseif (humanoid.FloorMaterial == Enum.Material.Fabric) then
				(sounds.Running :: any).Pitch = 1
				sounds.Running.SoundId = "rbxassetid://9083849830"
			else
